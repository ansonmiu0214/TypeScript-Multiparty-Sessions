import { Svr, Session } from "./CompactCalculator/Svr";

const delay = (ms: number) => new Promise((resolve, _) => {
...
const handler = (sessionID: string) => {
  ...
  const handleRequest = Session.Initial({
    Add: async (Next, x, y) => {
      await delay(5000);
      console.log(`${sessionID}: addition of ${x} and ${y}`);
      return Next.Res([x + y], handleRequest);
    },
    Mult: (Next, x, y) => {
      console.log(`${sessionID}: multiplication of ${x} and ${y}`);
      return Next.Res([x * y], handleRequest);
    },
    Double: (Next, x) => {
      console.log(`${sessionID}: doubling of ${x}`);
      return Next.Res([2*x], handleRequest);
    },
    Quit: async (Next) => {
      await delay(3000);
      console.log(`${sessionID}: quit`);
      return Next.Terminate([], Session.Terminal);
    }
  });
...
new Svr(
  ...
  (sessionID, role, reason) => {
    console.error(`${sessionID}: ${role} cancelled because: ${reason}`);
  },
  handler,
);
